...
pub struct LoadedSession {
    /// The normalized session that was loaded or created.
    pub session: CreativeSession,
    /// Whether a generation was used instead of the active file.
    pub recovered_from_generation: bool,
}

/// Error returned when the active session cannot be loaded safely.
#[derive(Debug)]
...
impl SessionStore {
    ...
    ///
    /// # Errors
    /// Returns an error when the active session and every generation are
    ...
    /// Saves a normalized session using generation backup and atomic replace.
    ///
    /// # Errors
    /// Returns an I/O or validation error without replacing the active file.
    ...
    /// Lists readable generation metadata from newest to oldest.
    ...
    /// Restores one validated generation as the active session.
    ///
    /// # Errors
    /// Returns an error for an unsafe generation name or an invalid generation
    /// payload.
...
pub struct RecoveryCandidate {
    ...
    /// Session update timestamp stored in the generation.
    pub updated_at_ms: u64,
    /// Session identity stored in the generation.
    pub session_id: String,
    /// Optional project display name.
    pub project_name: Option<String>,
    /// Session note stored in the generation.
    pub note: String,
}
