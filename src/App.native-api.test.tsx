...
describe("App driven by FakeNativeApi", () => {
  ...
  it("keeps output safe when the device is faulted and recovers into emergency mute", async () => {
    const fake = new FakeNativeApi({ audio: fakeAudioStatus({ state: "faulted", message: "Device disconnected." }) });
    renderApp(fake);
    await waitForAppShell();
  ...
  it("does not lose the Scratch Session when the workspace switches", async () => {
    ...
    await user.click(within(workspaceNav).getByRole("button", { name: /Play/ }));
    expect((screen.getByPlaceholderText(/意図、比較対象/) as HTMLTextAreaElement).value).toContain("remember this tone");

    await waitFor(() => {
      expect(fake.savedSessions.some((session) => session.note.includes("remember this tone"))).toBe(true);
    });
  });
  ...
  it("previews a sample pad through React props, not DOM listeners", async () => {
    ...
    await waitFor(() => {
      expect(fake.calls.filter((call) => call === "previewSample").length).toBeGreaterThan(previewCallsBefore);
    });
  });

  it("loads a VST3 into the rack through the injected api and projects it into the Scratch Session", async () => {
    const plugins: PluginEntry[] = [
      { id: "plug:example", name: "Example Synth", vendor: "Acme", version: null, format: "VST3", path: "C:\\VST3\\example.vst3", bundle: false, modifiedAtMs: null, scanState: "validated" },
      { id: "plug:other", name: "Other Synth", vendor: "Acme", version: null, format: "VST3", path: "C:\\VST3\\other.vst3", bundle: false, modifiedAtMs: null, scanState: "validated" },
    ];
