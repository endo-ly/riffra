...
describe("AudioDriverPicker", () => {
  it("shows the effective device values instead of pretending a preset value is active", () => {
    render(<AudioDriverPicker probe={probe} current="Windows Audio" sampleRate={48_000} bufferSize={480} onSelect={() => undefined} />);

    expect(screen.getByRole("button", { name: "48,000 Hz" })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: "480 samples" })).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByRole("button", { name: "64 samples" })).toHaveAttribute("aria-pressed", "false");
  });

  it("requests a new buffer with the current driver and effective sample rate", async () => {
    const onSelect = vi.fn();
    const user = userEvent.setup();
    render(<AudioDriverPicker probe={probe} current="Windows Audio" sampleRate={48_000} bufferSize={480} onSelect={onSelect} />);
