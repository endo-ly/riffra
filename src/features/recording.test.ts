...
import { decideRecordingToggle } from "./recording";
...
describe("decideRecordingToggle", () => {
  ...
  it("ignores presses while a command is in flight", () => {
    ...
    expect(decideRecordingToggle({ ...base, commandPending: true, recordingActive: true }).kind).toBe("ignore");
  });

  it("cancels an armed countdown before anything else", () => {
    expect(decideRecordingToggle({ ...base, countdown: 4, recordingActive: false }).kind).toBe("cancelCountdown");
    expect(decideRecordingToggle({ ...base, countdown: 0, recordingActive: true }).kind).toBe("cancelCountdown");
  });

  it("stops an active recording before starting a new one", () => {
    expect(decideRecordingToggle({ ...base, recordingActive: true, countInBeats: 4 }).kind).toBe("stop");
  });
