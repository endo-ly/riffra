...
function TimelineRenderControls({ session, result, message, onRender, onPreview, onStop, previewing }: { session: ScratchSession; result: RenderResult | null; message: string; onRender: (options: RenderOptions) => void; onPreview: () => void; onStop: () => void; previewing: boolean }) {
...
function ReferenceCompare({ analysis, recordings, references, referenceId, onSelect, onPreview, onStop, previewingId }: { analysis: AudioAnalysis | null; recordings: RecordingAsset[]; references: Record<string, AudioAnalysis>; referenceId: string | null; onSelect: (recording: RecordingAsset) => void; onPreview: (recording: RecordingAsset) => void; onStop: () => void; previewingId: string | null }) {
  ...
  return (
    <main className={`app-shell ${focusMode ? "focus-mode" : ""} ${isMuted ? "is-muted" : ""}`}>
      ...
      <section className="workspace">
        ...
        {session.workspace === "analyze" && <><WorkspaceAnalyze analysis={analysis} /><ReferenceSuggestion analysis={analysis} recordings={recordings} references={referenceAnalyses} referenceId={referenceId} session={session} setSession={setSession} onSelect={(recording) => void selectReference(recording)} onPreview={(recording) => void previewReference(recording)} onStop={() => void stopReferencePreview()} previewingId={referencePreviewingId} /></>}
