...
function ReferenceCompare({ analysis, recordings, references, referenceId, onSelect, onPreview, onStop, onSyncPreview, previewingId, syncPreviewing }: { analysis: AudioAnalysis | null; recordings: RecordingAsset[]; references: Record<string, AudioAnalysis>; referenceId: string | null; onSelect: (recording: RecordingAsset) => void; onPreview: (recording: RecordingAsset) => void; onStop: () => void; onSyncPreview: () => void; previewingId: string | null; syncPreviewing: boolean }) {
...
function ReferenceSuggestion({ analysis, recordings, references, referenceId, session, setSession, onSelect, onPreview, onStop, onSyncPreview, previewingId, syncPreviewing }: { analysis: AudioAnalysis | null; recordings: RecordingAsset[]; references: Record<string, AudioAnalysis>; referenceId: string | null; session: ScratchSession; setSession: (value: ScratchSession) => void; onSelect: (recording: RecordingAsset) => void; onPreview: (recording: RecordingAsset) => void; onStop: () => void; onSyncPreview: () => void; previewingId: string | null; syncPreviewing: boolean }) {
  ...
  return <>
    ...
    <ReferenceCompare analysis={analysis} recordings={recordings} references={references} referenceId={referenceId} onSelect={onSelect} onPreview={onPreview} onStop={onStop} onSyncPreview={onSyncPreview} previewingId={previewingId} syncPreviewing={syncPreviewing} />
...
function App() {
  ...
  const previewReferencePair = useCallback(async () => {
    ...
    await stopSamplePreview();
    await previewSample(analysis.path, 0, 0);
    setAudio(await previewSample(referencePath, 0, 0));
    setReferencePreviewingId(null);
    setReferenceSyncPreviewing(true);
  }, [analysis, recordings, referenceId]);
  ...
  return (
    <main className={`app-shell ${focusMode ? "focus-mode" : ""} ${isMuted ? "is-muted" : ""}`}>
      ...
      <section className="workspace">
        ...
        {session.workspace === "analyze" && <><WorkspaceAnalyze analysis={analysis} /><ReferenceSuggestion analysis={analysis} recordings={recordings} references={referenceAnalyses} referenceId={referenceId} session={session} setSession={setSession} onSelect={(recording) => void selectReference(recording)} onPreview={(recording) => void previewReference(recording)} onStop={() => void stopReferencePreview()} onSyncPreview={() => void previewReferencePair()} previewingId={referencePreviewingId} syncPreviewing={referenceSyncPreviewing} /></>}
