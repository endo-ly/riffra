...
export function CaptureSettings({
  ...
  setSession: (value: ScratchSession) => void;
}) {
  return (
...
function WorkspaceSample({
  ...
  onPreviewPad: (pad: ScratchSession['samplePads'][number]) => void;
}) {
...
function SamplePreviewControls({
  session,
  playingId,
  onPreview,
  onStop,
  ...
  onPreview: (pad: ScratchSession['samplePads'][number]) => void;
  onStop: () => void;
}) {
  if (!session.samplePads.length) return null;
  return (
...
function TimelineRenderControls({
  session,
  result,
  stems,
  message,
          ...
          <label className="ai-permission">
            ...
            <select
              ...
              onChange={(event) =>
                setSession({
                  ...session,
                  aiPermission: event.target.value as ScratchSession['aiPermission'],
                })
              }
            >
    ...
    async (slot: 'A' | 'B') => {
      ...
      if (plugin) {
        let nextAudio = plugin.stateData
          ? await setPluginState(plugin.stateData)
          : await setPluginBypassed(plugin.bypassed);
  ...
  return (
    <main className={`app-shell ${focusMode ? 'focus-mode' : ''} ${isMuted ? 'is-muted' : ''}`}>
      <header className="global-bar">
        ...
        <button className="session-title" onClick={renameSession} title="Rename Scratch Session">
      ...
      <aside className="library-panel">
        ...
        <div className="library-content">
          ...
          {query && (
            <section className="library-search-results">
              <span className="eyebrow">CROSS-ASSET SEARCH · {libraryResults.length}</span>
              {libraryResults.slice(0, 8).map((asset) => (
                <button
                ...
                <div className="library-asset-detail">
                  <header>
                    ...
                    <div>
                      <button
                        className="text-button"
                        disabled={!selectedLibraryAsset.path?.toLowerCase().endsWith('.wav')}
                        onClick={() => void previewSelectedLibraryAsset()}
                      >
                        Preview
                      </button>
                      <button
                        className="text-button"
                        onClick={() => void editSelectedLibraryAsset()}
                      >
                        Edit
                      </button>
                    </div>
                  </header>
                  <small>Tag: {selectedLibraryAsset.tag ?? '—'}</small>
                  <p>{selectedLibraryAsset.note ?? 'No note yet.'}</p>
          ...
          {librarySection === 'Plugins' ? (
            <>
              <small className="scan-message">{visiblePlugins.length}件を表示</small>
              {visiblePlugins.slice(0, 12).map((plugin) => (
                <button
                  className="plugin-row"
                  key={plugin.id}
                  onClick={() => void loadPluginIntoRack(plugin)}
            ...
            <>
              {visibleRecordings.slice(0, 12).map((recording) => (
                <button
                  className="plugin-row recording-row"
                  key={recording.id}
                  disabled={Boolean(recording.error)}
                  onClick={() => void openRecordingAnalysis(recording)}
        ...
        <button className="inbox-button" onClick={() => setLibrarySection('Recordings')}>
          <span className="inbox-icon">↓</span>
          <div>
            <strong>Inbox</strong>
            <small>{recordings.length} items</small>
          </div>
        </button>
      </aside>
      ...
      <section className="workspace">
        ...
        {session.workspace === 'sample' && (
          <>
            ...
            />
            <MidiDevices probe={midi} onRefresh={() => void probeMidiDevices().then(setMidi)} />
            <MidiMonitor
              probe={midi}
              audio={audio}
      ...
      <footer className="transport">
        <div className="transport-left">
          ...
          <button
            ...
            onClick={() => void (transportPlaying ? stopTransport() : playTransport())}
          >
            <Icon name={transportPlaying ? 'stop' : 'play'} />
          </button>
          <button aria-label="Stop" onClick={() => void stopTransport()}>
            <Icon name="stop" />
          </button>
          <button
        ...
        <div className="master">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              void setMasterGainDb(gainDb).then(setAudio);
            }}
          />
        </div>
