...
この文書は、`Windows統合音楽制作ワークベンチ_製品仕様書_改訂版.md` を、実装可能な要求単位へ分解したものです。画面のモックや実装状況ではなく、ユーザーが行う操作と、製品が守るべき結果を記述します。

要求IDは、仕様変更・設計・実装・検証の会話で同じ対象を指すための名前です。現在の実装状況や未解決事項は [spec-traceability.md](./spec-traceability.md) へ分離し、要求そのものを進捗で書き換えないようにします。
