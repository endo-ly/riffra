...
Audio Clipは所属Trackの`audioClips`配列に入れ、MIDI Clipは`midiClips`配列に入れる。MIDI Clipの`durationTicks`はAudioのFrame変換を行わず、そのまま音楽時間として扱う。Track Rackは各Trackの`rack`から復元し、Track単位のDSPとGain/Pan/Mute/Soloを経由してMasterへ加算する。現行Native Runtimeは1 Trackにつき有効Plugin 1個を処理する。
