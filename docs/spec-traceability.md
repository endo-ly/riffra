...
| D. Arrange | timeline engine + offline render | In progress | non-destructive Inbox audio clip references with editable position/gain/fade/pan/mute and a new stereo float WAV render with bounded master gain; MIDI edit, PDC and stem export remain |
| E. Sample | sampler preview bus + MIDI monitor | In progress | non-destructive source-to-pad mapping with editable slice range, native safety-limited WAV audition, mouse/Z–M keyboard pad audition, Windows MIDI port discovery, explicit input monitoring, preloaded MIDI-triggered pad playback with trigger counters, and MIDI event sidecars alongside Quick Record; polyphony and MIDI clip editing remain |
...
| I. Creative Memory | library catalog + provenance manifests | In progress | VST3 catalog, recording-manifest search/listing, SQLite cross-asset index/search, rack/session provenance sidecars and portable session export that collects referenced audio into an assets folder are available; richer tags, related-asset traversal and non-destructive preview remain |
| J. Recovery | supervisor + autosave generations | In progress | corrupted current save fallback, explicit `--safe-mode` startup isolation, persisted VST3 placeholders marked Missing dependency, and selectable stable autosave generations; discard-recovery and richer crash diagnostics remain |
