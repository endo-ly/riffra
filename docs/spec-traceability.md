...
| C. Capture | audio sidecar + recording journal | In progress | JUCE threaded Raw/Processed WAV, completed/recoverable manifest, Inbox recording index, persisted visual 0–8 beat count-in, and synthetic self-test; hardware interruption matrix remains |
| D. Arrange | timeline engine + offline render | In progress | non-destructive Inbox audio clip references with editable position/source range/loop/gain/fade/pan/mute, persisted track lanes with mute/solo/mix state, duplicate/split clip operations, a persisted basic MIDI clip/piano-roll editor with standard MIDI export, bounded master/track range-normalized stereo float WAV render, independent per-track stem export, and safe rendered-audio preview; PDC remains |
| E. Sample | sampler preview bus + MIDI monitor | In progress | non-destructive source-to-pad mapping with editable slice range/gain/loop, native safety-limited WAV audition, mouse/Z–M keyboard pad audition, Windows MIDI port discovery, explicit input monitoring and Panic, eight-voice MIDI-triggered pad playback with per-note release, a bounded sine fallback for unmapped notes, trigger counters, and MIDI event sidecars alongside Quick Record; full keyboard instrument/polyphonic synth remains |
| F. Analyze | analysis workers | In progress | offline WAV 128-bin waveform, peak/true-peak estimate/RMS, clipping and dynamic-range facts, duration, zero crossings, simple spectrum peak, stereo phase correlation, read-only Reference delta/loudness-match metrics, and synchronized-start dual-preview controls with a loop toggle; richer time-aligned reference editing remains |
...
| H. AI | reversible suggestion service | In progress | offline Reference ChangeSet preview shows target/current/proposed/reason/effect/risk with selected apply, reject and Undo coverage; persisted Explain/Suggest/Apply permission, selectable local context, and applied ChangeSet history are available; provider permissions and external-send control remain |
