...
| E. Sample | sampler preview bus + MIDI monitor | In progress | non-destructive source-to-pad mapping with editable slice range, native safety-limited WAV audition, Windows MIDI port discovery, explicit input monitoring and preloaded MIDI-triggered pad playback with trigger counters; keyboard sampler, polyphony and MIDI recording remain |
