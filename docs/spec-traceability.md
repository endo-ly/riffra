...
| B. Tone Design | plugin workers + rack engine | In progress | isolated real VST3 load/process, persisted rack path, explicit safety-preserving bypass, common parameter view for live VST3 controls, and session A/B rack snapshots; complete plugin state persistence, macro/parallel rack and matched A/B remain |
