...
| B. Tone Design | plugin workers + rack engine | In progress | isolated real VST3 load/process, persisted rack path and parameter values, explicit safety-preserving bypass, common parameter view, mapped normalized macros, and session A/B rack snapshots; complete plugin state persistence, parallel rack and matched A/B remain |
...
| F. Analyze | analysis workers | In progress | offline WAV 128-bin waveform, peak/true-peak estimate/RMS, clipping and dynamic-range facts, duration, zero crossings, simple spectrum peak, stereo phase correlation and read-only Reference delta/loudness-match metrics; synchronized reference playback remains |
...
| I. Creative Memory | library catalog + provenance manifests | In progress | VST3 catalog, recording-manifest search/listing, SQLite cross-asset index/search, editable tag/note metadata, related-asset traversal, non-destructive WAV preview, rack/session provenance sidecars and portable session export that collects referenced audio into an assets folder are available |
