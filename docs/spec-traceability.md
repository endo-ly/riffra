...
| --- | --- | --- | --- |
| A. Instant Play | audio sidecar + session recovery | In progress | emergency-muted startup, muted device recovery with rack reprepare, validated rack restoration request, native input/output peak metering and Windows WASAPI/ASIO/MIDI device discovery; device selection and cold/warm timing remain |
...
| E. Sample | sampler preview bus + MIDI probe | In progress | non-destructive source-to-pad mapping with editable slice range, native safety-limited WAV audition and Windows MIDI port discovery; keyboard sampler and MIDI play/record remain |
...
| I. Creative Memory | library catalog + provenance manifests | In progress | VST3 catalog, recording-manifest search/listing, rack/session provenance sidecars and explicit portable session-manifest export are available; SQLite cross-asset index, related assets and non-destructive preview remain |
