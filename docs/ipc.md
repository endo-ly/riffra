...
- `sendTrackMidi` と `panicTrackMidi` は、要求に含まれる Track ID へ直接ライブMIDIを送る。Play Surface の表示状態はMIDIルーティングの状態を持たず、Timeline由来のPDCとライブ入力の経路はTrack Runtime内で分離して扱う。`reset_feedback_protection` はフィードバック保護だけを明示的に解除する
