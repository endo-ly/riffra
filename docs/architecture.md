...
読み込んだプラグインは音声子プロセス内で処理する。プラグインを含むArrangement編集は、候補Snapshotの準備が成功してからcanonical Sessionへ保存する。候補の生成に失敗した場合はSessionを変更せず、保存後の失敗時は直前のRuntime Graphへ戻す。インスタンス生成・初期化・破棄とArrangement Snapshotの準備は音声コールバックおよびコマンド受付から分離する。画面側はラック状態と操作を管理し、音声バッファーを直接扱わない。
