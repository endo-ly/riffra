...
| AudioDeviceProbe                                       | オーディオ/MIDIデバイスのprobing結果。ドライバごとに入出力デバイスとチャンネル名・インデックスを持つ | `apps/desktop/src/lib/domain.ts` | `apps/desktop/src-tauri/src/model.rs` | `native/audio-engine/src/AudioMain.cpp`（probeAudioDevices）                  |
...
| AudioDeviceInfo / AudioChannelInfo                     | オーディオデバイスと、そのデバイスで選択可能なチャンネル                                             | `apps/desktop/src/lib/domain.ts` | `apps/desktop/src-tauri/src/model.rs` | `native/audio-engine/src/AudioMain.cpp`                                       |
