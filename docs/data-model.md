...
| DeviceChannels                     | 選択デバイスの遅延チャンネル詳細。Audio Settingsで対象デバイスを1回だけ開いて入出力チャンネル名を返す           | `apps/desktop/src/lib/domain.ts` | `apps/desktop/src-tauri/src/model.rs` | `native/audio-engine/src/AudioMain.cpp`（probeDeviceChannels）                |
