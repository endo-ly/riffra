...
| エンティティ                      | 役割                                                                                                               | TS                               | Rust                                | C++                                                    |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------- | ----------------------------------- | ------------------------------------------------------ |
| CreativeSession                   | 制作中の状態の正準モデル。ワークスペース、デザイン文脈、ライブ状態、アレンジ、ラック、スナップショット、設定を所有 | `apps/desktop/src/lib/domain.ts` | `crates/riffra-core/src/session.rs` | —                                                      |
| Workspace                         | 2つの固定制作領域（design / arrange）。演奏入力はArrange内のPlay Surfaceが担う                                     | `apps/desktop/src/lib/domain.ts` | `crates/riffra-core/src/session.rs` | —                                                      |
...
| PlayState                         | サンプルインストゥルメント（sample_instrument）のパッド状態                                                        | `apps/desktop/src/lib/domain.ts` | `crates/riffra-core/src/session.rs` | —                                                      |
...
| AudioDeviceProbe                   | オーディオデバイスのpassive probing結果。ドライバごとに入出力デバイスを持つ（起動時はチャンネル不在・開かない）         | `apps/desktop/src/lib/domain.ts` | `apps/desktop/src-tauri/src/model.rs` | `native/audio-engine/src/AudioMain.cpp`（discoverAudioDevices）               |
| DeviceChannels                     | 選択デバイスの遅延チャンネル詳細。AudioStatusに存在しない組み合わせだけをAudio Settingsから取得し、失敗時は明示的に返す | `apps/desktop/src/lib/domain.ts` | `apps/desktop/src-tauri/src/model.rs` | `native/audio-engine/src/AudioMain.cpp`（probeDeviceChannels）                |
| MidiProbe                          | MIDIデバイスのprobing結果（独立・オーディオドライバに非接触）                                                           | `apps/desktop/src/lib/domain.ts` | `apps/desktop/src-tauri/src/model.rs` | `native/audio-engine/src/AudioMain.cpp`（discoverMidiDevices）                |
...
`CreativeSession::validate_and_normalize()`（`crates/riffra-core/src/session.rs`）が保存・読込時に強制する。各エンティティが常に満たす条件を以下に示す。
...
| MidiClip.events                        | 200,000 / クリップ                                               |
| Arrangement.recording_sessions / takes | 256 / アレンジ                                                   |
...
- `MIDI Clip`はInstrument Trackにのみ所属できる。Audio用Frame/Sample Rateへ変換しない
- MIDI EventはNoteへ正規化せず、ControlChange、PitchBend、ChannelPressureのデータを保持する
- `MIDI Clip`のMove、Trim、Split、Duplicate、Mute、LoopはArrangementの一回のCommitとして確定する
- Recording TakeのRaw/Processed切替は同じTakeのTimeline位置、Trim、Gain、Pan、Fadeを共有する
- Loop Recordingは周回ごとにRecordingTakeRecordを作り、最後のTakeだけをActiveとして表示する
- Punch Range外の入力はRecording Sessionへ書き込まず、元Clipを変更しない
