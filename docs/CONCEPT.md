...
| ヘッドレスCLI   | スクリプト、AIエージェント | プロジェクト操作、トラック・クリップ編集、MIDI配置 |

CLIはGUIの操作を模したものではなく、同じriffra-coreのドメインモデルを直接操作する。AIエージェントは `riffra` をサブプロセスとして起動し、標準入出力のJSON Linesで対話する。ライブ音声を必要としない作業（編集、MIDI配置、プロジェクト整理）を自動化できる。
