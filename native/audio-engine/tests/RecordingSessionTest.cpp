...
{
    juce::String error;
    auto session = RecordingSession::create(
        directory.get(), kSampleRate, kChannels, kChannels, error);
...
TEST_F(RecordingSessionTest, WritesExpectedRecordingManifest)
{
    ...
    EXPECT_EQ(static_cast<juce::int64>(manifest.getProperty("samplesWritten", -1)),
              kTotalSamples);
...
TEST_F(RecordingSessionTest, PreservesIncompleteRecordingAsRecoverable)
{
    ...
    juce::String error;
    auto session = RecordingSession::create(
        incompleteDirectory, kSampleRate, kChannels, kChannels, error);
    ASSERT_NE(session, nullptr) << error;
