...
struct TimelineTimebase final {
    ...
    [[nodiscard]] std::int64_t tickToSample(
        const std::uint64_t tick,
        const double sampleRate) const noexcept {
        if (ppq == 0 || !std::isfinite(bpm) || bpm <= 0.0
    ...
    [[nodiscard]] std::uint64_t sampleToTick(
        ...
        return static_cast<std::uint64_t>(std::llround(
            static_cast<double>(sample) * bpm * static_cast<double>(ppq)
            / (sampleRate * 60.0)));
    }
};
