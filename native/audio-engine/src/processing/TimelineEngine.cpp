...
bool TimelineEngine::loadSnapshot(const juce::var& snapshot, juce::AudioFormatManager& formats,
    ...
    if (!prepareSnapshot(snapshot, formats, outputSampleRate, maximumBlockSize, prepared,
...
bool TimelineEngine::prepareSnapshot(const juce::var& snapshot, juce::AudioFormatManager& formats,
                                     const double outputSampleRate, const int maximumBlockSize,
                                     std::unique_ptr<PreparedTimeline>& prepared,
    ...
    for (const auto& trackValue : *tracks.getArray()) {
        ...
        for (const auto& value : *clips.getArray()) {
            ...
            clip->fadeShape = juce::jlimit(0, 2, static_cast<int>(value.getProperty("fadeShape", 1)));
            ...
            if (!offlineMode) {
                clip->bufferingSource = std::make_unique<juce::BufferingAudioSource>(
                    clip->readerSource.get(), readAheadThread, false, kReadAheadSamples, 2);
    ...
    for (auto& clipPtr : track.clips) {
        ...
        while (remaining > 0) {
            ...
            for (int sample = 0; sample < chunk; ++sample) {
                ...
                auto envelope = 1.0f;
                auto shape = [](float p, int fadeShape) {
                    switch (fadeShape) {
                        case 0: return p;
                        case 2: return p * p * (3.0f - 2.0f * p);
                        default: return std::sin(juce::MathConstants<float>::halfPi * p);
                    }
                };
                if (clip.fadeInSamples > 0 && position < clip.fadeInSamples) {
                    const auto progress =
                        static_cast<float>(position) / static_cast<float>(clip.fadeInSamples);
                    envelope = std::min(envelope, shape(progress, clip.fadeShape));
                }
